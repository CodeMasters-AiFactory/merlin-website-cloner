/**
 * Direct test of the WebsiteCloner with fixed settings
 */
...
async function main() {
  ...
  console.log('DIRECT CLONER TEST - JETON.COM');
  console.log('='.repeat(60));

  // Clean output dir
  const outputDir = './direct-test-jeton';
  try {
  ...
  try {
    const result = await cloner.clone({
      ...
      optimizeImages: false, // Skip for speed
      onProgress: (progress) => {
    ...
    if (result.verificationResult) {
      console.log(`\nVerification Score: ${result.verificationResult.score}%`);
    }
